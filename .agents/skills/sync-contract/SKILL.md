...
1. Update the marker fence and the YAML authoring form in the contract
   file.
2. Update the `MARKER` constant in `scripts/validate_repo.py`, the
   `NEAR_MISS` prefix beside it, and the self-test fixtures.
...
5. Run `just check`. Checks D1, D3, and A3 name every copy still out of
   line; iterate until green — the validator, not a remembered list, is the
   inventory.
