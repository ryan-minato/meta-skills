...
1. Dry run: `python3 scripts/dispose.py`. It reads the marker from this
   ...
   symlinked entries are reported, not followed.
2. Show the listing to the user verbatim and ask for fresh, explicit
   ...
   anything.
3. On confirmation, run the same command plus `--delete`. It deletes
   every matched directory, this skill's own directory last, and prints
   a deletion report.
