#!/usr/bin/env python3
"""Discover catalogs and skills from the live meta-skills repository.

Examples:
  python3 scripts/discover.py
  python3 scripts/discover.py --catalog core --full
  python3 scripts/discover.py --skill data-science/meta-ds-project --full
...
ARCHIVE_URL = (
    "https://codeload.github.com/ryan-minato/meta-skills/tar.gz/refs/heads/main"
)
...
SKILL_RE = re.compile(r"^meta-[a-z0-9]+(?:-[a-z0-9]+)*$")
        ...
        for skill_index, raw_skill in enumerate(raw_skills):
            ...
            if not skill_ref.startswith("./"):
                raise DiscoveryError(
                    ...
                    "`./` and name one direct skill directory."
                )
            ...
            if "/" in skill_name or not SKILL_RE.fullmatch(skill_name):
                raise DiscoveryError(
                    ...
                    "direct `meta-*` skill directory; path traversal and nested "
                    "paths are forbidden."
                )
    ...
    manifest = {
        "plugins": [
            ...
            },
            {
                "name": "other",
                "source": "./skills/other",
                "description": "Other test catalog.",
                "skills": ["./meta-two"],
            },
        ]
    }
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        def materialize(raw_manifest: object = manifest) -> None:
            ...
            for relative, content in (
                ("skills/core/meta-one/SKILL.md", folded_skill),
                ("skills/other/meta-two/SKILL.md", plain_skill),
            ):
                path = root / relative
        ...
        selected = select_inventory(inventory, "core", None)
        assert len(selected) == 1 and selected[0].name == "core"
        cases += 1
        ...
        (root / "skills/other/meta-two/SKILL.md").unlink()
        try:
            load_inventory(root)
        except DiscoveryError as exc:
        ...
        escaping["plugins"][0]["skills"] = ["./../outside"]
        ...
        (root / "skills/other/meta-two/SKILL.md").write_text(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Example: python3 scripts/discover.py --catalog core --full\n"
            "The default source is the live main branch of "
            "ryan-minato/meta-skills."
        ),
    ...
    selector.add_argument(
        "--skill",
        help="return one exact dependency target as catalog/meta-skill",
    )
    parser.add_argument(
        "--full",
        action="store_true",
...
def main() -> int:
    ...
    if args.skill:
        ...
        if (
            ...
            parser.error("--skill must be a canonical `catalog/meta-skill` identifier")
