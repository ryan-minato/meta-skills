...
Read when the target embeds and retrieves: local ANN indexes or vector
databases. One line and an entry point per tool; fetch install and
deployment details from the entry point. The databases deploy as
services (containers or managed); the ANN libraries install as packages.
