...
discovery — takes the unsuffixed `meta-ml-<name>`. A `-docs` skill maps
only the tools its tables list; discovering docs for a package it does not
cover is out of scope — the agent reaches for a dedicated discovery skill
on its own, so no `-docs` skill carries or depends on that procedure.
...
`llms.txt` plain-text index (agent-preferred: fetch a page's `.md` source
or the `llms.txt` index rather than HTML; `llms-full.txt` is a whole-site
dump to search programmatically, never read whole) are marked; re-probe the
others when refreshing this table. PyPI packages install with `pip install <package>` (or the
