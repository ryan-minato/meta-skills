# Core Vision Libraries & Pretrained Models

Read when any general-purpose vision library, pretrained-model zoo, or
augmentation library is in play. One line and an entry point per tool;
fetch install commands and API details from the entry point. No entry is
a recommendation.
