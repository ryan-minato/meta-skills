...
def check_pyproject(root: Path, issues: list[Issue]) -> None:
    ...
    if "project" not in data:
        add(
            ...
            "pyproject.project",
            path.name,
            "Missing [project] table from uv package initialization.",
        )
    if "build-system" not in data:
        add(
            ...
            "pyproject.package",
            path.name,
            "Missing [build-system]. Initialize a package/src layout with uv init --package.",
        )
    tool = data.get("tool", {})
    for name in ("ruff", "pytest"):
        if name not in tool and not (name == "pytest" and "pytest.ini_options" in tool):
            add(
                issues,
                f"pyproject.{name}",
                path.name,
                f"Missing {name} tool configuration. Merge the scaffold tool tables.",
            )
    forbidden = {"mypy", "pyright", "ty"}
    for name in forbidden & set(tool):
        add(
            issues,
            "pyproject.type-checker",
            path.name,
            f"Type checker [tool.{name}] is outside this scaffold. Remove it unless the user explicitly requested it.",
        )
    serialized = json.dumps(data).lower()
    for token in ("cov-fail-under", "fail_under"):
        if token in serialized:
            add(
                issues,
                "pyproject.coverage-target",
                path.name,
                "Coverage threshold found. Remove it; tests protect risky custom logic, not a target percentage.",
            )
    dependencies = flatten_strings(data.get("project", {}).get("dependencies", []))
    dependencies += flatten_strings(data.get("dependency-groups", {}))
    joined = " ".join(dependencies)
    for package in ("loguru", "pydantic-settings", "pytest", "ruff", "pre-commit"):
        if package not in joined:
            add(
                issues,
                "pyproject.dependency",
                path.name,
                f"Missing required dependency {package!r}. Add it with uv in the appropriate group.",
            )
...
def check_settings_and_workflows(root: Path, issues: list[Issue]) -> None:
    ...
    if len(package_dirs) != 1:
        add(
            ...
            "python.src-layout",
            "src",
            "Expected exactly one importable package under src/. Initialize it with uv init --package.",
        )
    settings_files = [
        path / "settings.py"
    ...
    if len(settings_files) != 1:
        add(
            ...
            "python.settings",
            "src",
            "Expected exactly one src package with settings.py. Keep one package-level settings module.",
        )
    else:
        text = read_text(settings_files[0], issues, "python.settings-read")
        for token in ("pydantic_settings", "TomlConfigSettingsSource", ".env"):
            if token not in text:
                add(
                    issues,
                    "python.settings-contract",
                    settings_files[0].relative_to(root),
                    f"Settings module does not show {token!r}. Load TOML and environment overrides with Pydantic Settings.",
                )
    ...
    if not workflow_files:
        ...
        "loguru" in read_text(path, issues, "python.workflow-read")
        ...
        add(
            ...
            "python.logging",
            "src",
            "Workflow entries do not initialize Loguru. Bind run/workflow/step context.",
        )
...
def check_commands_and_hooks(root: Path, issues: list[Issue]) -> None:
    justfile = read_text(root / "justfile", issues, "just.read")
    recipes = set(
    ...
    hooks = read_text(
        root / ".pre-commit-config.yaml", issues, "pre-commit.read"
    ).lower()
    for token in ("ruff", "pytest", "gitleaks"):
        if token not in hooks:
            add(
                issues,
                "pre-commit.hook",
                ".pre-commit-config.yaml",
                f"Missing {token} hook. Pin its current upstream revision.",
            )
